...
| **D7** | **The PR join is two-path** | `pullRequestForWorktree` (`git_map.go`) first tries `direct`, the issue-key-derived cache lookup (`issuekey.FromBranch(branch)` → `pr:<slug>#<N>`), but only accepts it when `direct.HeadRefName == branch` — PR #N and issue #N are different nodes in the same repo, so a hit on the wrong node must be rejected. It then falls back to a scan of `github.CachedPRs(owner, repo)` for the exact `HeadRefName`. Neither path filters by state: a closed or merged PR on the branch is still returned, because the sidebar renders PR state. | Trusting the issue-key-derived cache hit alone once it's non-nil — silently returns the wrong PR whenever the branch's derived issue number happens to also be a PR number that isn't this branch's PR. |
...
- **AC-GIT-PR-JOIN** — `worktree.pullRequest` resolves via the two-path join (D7); state is never
  filtered.
