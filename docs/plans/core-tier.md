...
Core-tier acceptance criteria. **Primary evidence is use-proof: an observation from the running binary this turn** (unit/`go test` runs stay as regression backfill, not as the AC evidence). Threshold `T` = lag threshold from config (test uses a short value); interval `I` = canary/health interval.
